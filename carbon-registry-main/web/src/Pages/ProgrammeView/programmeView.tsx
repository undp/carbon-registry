...
const ProgrammeView = () => {
  ...
  const [authoriseDoc2Url, setauthoriseDoc2Url] = useState<any>('');
  ...
  const [uploadMonitoringReport, setUploadMonitoringReport] = useState<boolean>(true);
  ...
  const [reportverificationnb, setreportverificationnb] = useState(0);
  const { id } = useParams();
  ...
  // Generer une lettre d'autorisation non sigée
  const generateletter = () => {
    try {
      ...
      const url = 'https://docregister.ci.skyvisionafrica.com/api/v1/doc/create_project';

      const requestData: any = {
        IdProjet: data?.programmeId,
        Type: 'autorisation_letter',
        Lang: 'fr',
      };
      ...
      axios({
        ...
        if (response && response.data) {
          ...
          //Modifier l'url de la lettre d'autorisation
          setauthoriseDoc2Url(response.data.data.Url);
        }
      });
      setOpenModal(false);
      message.open({
  ...
  const getreportingverificationdoc = async (programmeId: string) => {
    try {
      ...
      if (response.data) {
        ...
        setreportverificationnb(response.data.length);
  ...
  const updateProgrammeData = async (response: any) => {
  ...
  const onPopupAction = async (
    ...
    let error;
    if (endpoint === 'authorize2') {
      // Generer une lettre non signée
      // Modifier le doc url
      generateletter();
    } else {
      try {
  ...
  useEffect(() => {
    if (data) {
      ...
      getreportingverificationdoc(data?.programmeId);
  ...
  if (userInfoState?.userRole !== 'ViewOnly') {
    ...
    if (
      ...
      if (userInfoState?.companyRole === CompanyRole.GOVERNMENT || ministryLevelPermission) {
        ...
        if(reportverificationnb > 0 && authoriseDoc2Url ==''){
        actionBtns.push(
          <Button
            danger
            onClick={() => {
            ...
            <Card className="card-container">
              <div>
                <ProgrammeDocuments
                  ...
                  programmeId={data?.programmeId}
                  programmeOwnerId={programmeOwnerId}
                  authoriseDoc2Url={authoriseDoc2Url}
                  setauthoriseDoc2Url={setauthoriseDoc2Url}
                  programme={data}
                  onPopupAction={onPopupAction}
                  updateProgrammeData={updateProgrammeData}
                  getDocumentDetails={() => {
                    getDocuments(data?.programmeId);
                  }}
                  getProgrammeById={() => {
