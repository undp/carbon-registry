...
export class ProgrammeService {
  ...
  async transferApprove(
    ...
    if (
      req.companyIds &&
      req.companyIds.length > 1 &&
      (!req.companyCredit || req.companyCredit.length != req.companyIds.length)
    ) {
      throw new HttpException(
        "Incorrect company percentage",
        HttpStatus.BAD_REQUEST
      );
    }
    ...
    if (
      req.companyCredit &&
      transfer.creditAmount != req.companyCredit.reduce((a, b) => a + b, 0)
    ) {
      throw new HttpException(
        "Incorrect company percentage",
        HttpStatus.BAD_REQUEST
  ...
  async transferRequest(req: ProgrammeTransferRequest, requester: User) {
    ...
    if (
      programme.creditBalance -
        (programme.creditFrozen
          ? programme.creditFrozen.reduce((a, b) => a + b, 0)
          : 0) <
      req.creditAmount
    ) {
      throw new HttpException(
    ...
    if (programme.companyId.includes(requester.companyId)) {
      throw new HttpException(
        "Cannot initiate transfers for already owned programmes",
        HttpStatus.BAD_REQUEST
      );
    }
  ...
  async certify(req: ProgrammeCertify, add: boolean, user: User) {
    ...
    const updated = await this.programmeLedger.updateCertifier(
      ...
      user.companyId,
      add,
      `${user.id}#${user.name}#${user.companyId}#${company.name}`
    );
  ...
  async retireProgramme(req: ProgrammeRetire, user: string) {
    this.logger.log(
      `Programme ${req.programmeId} retiring Comment: ${req.comment}`
    );
    const updated: any = await this.programmeLedger.retireProgramme(
      req.programmeId,
      req.reason,
      user
    );
    if (!updated) {
      return new BasicResponseDto(
        HttpStatus.BAD_REQUEST,
        `Does not found a programme in issued status for the given programme id ${req.programmeId}`
      );
    }
  ...
  async updateProgrammeStatus(
    ...
    expectedCurrentStatus: ProgrammeStage,
    user: string
  ) {
    this.logger.log(
      `Programme ${req.programmeId} status updating to ${status}. Comment: ${req.comment}`
    );
    if (status == ProgrammeStage.ISSUED) {
      ...
      if (!updated) {
        return new BasicResponseDto(
          HttpStatus.BAD_REQUEST,
          `Does not found a programme in ${expectedCurrentStatus} status for the given programme id ${req.programmeId}`
        );
      }
