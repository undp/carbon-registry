...
type ArticleKey =
  ...
  | "article6_4_trade"
  | "article6_2_trade"
  | "article6_MVC_ajuste_trade"
  | "article6_MVC_non_ajuste_trade";

interface SelectedArticle {
  article6_8_trade: boolean;
  article6_4_trade: boolean;
  article6_2_trade: boolean;
  article6_MVC_ajuste_trade: boolean;
  article6_MVC_non_ajuste_trade: boolean;
}
...
const validatePolygons = (polygons: PolygonData[]) => {
  const validPolygons: PolygonData[] = [];
  const invalidPolygons: PolygonData[] = [];

  polygons.forEach((polygonData) => {
    const isValid = polygonData.Polygone.every((polygon) =>
  ...
  return { validPolygons, invalidPolygons };
};
...
export const ProgrammeCreationComponent = (props: any) => {
  ...
  const [selectedCertificator, setSelectedCertificator] = useState<string>("");
...
*/
  ...
  const [certificatorList, setCertificatorList] = useState<any[]>([]);
  ...
  const [polygonProjet, setPolygonProjet] = useState<PolygonData[]>([]);
  const [invalidPolygons, setInvalidPolygons] = useState<PolygonData[]>([]);
  ...
  const handleFileUpload = (file: any) => {
    ...
    reader.onload = (e: any) => {
      ...
      const parsedData: PolygonData[] = worksheet.map((row: any) => ({
        Intitule: row.Zone,
        Polygone: [JSON.parse(row.polygone)],
      }));

      console.log(parsedData);
      const { validPolygons, invalidPolygons } = validatePolygons(parsedData);
  ...
  const getCertificator_list = async () => {
    setLoading(true);
    try {
      ...
      if (response.data) {
        ...
        setCertificatorList(response?.data);
      }
    } catch (error: any) {
      console.log("Error in getting certificator data", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const onFinishStepOne = async (values: any) => {
    ...
    if (totalPercentage !== 100) {
      ...
      programmeDetails = {
        ...
        article6_MVC_non_ajuste_trade:
          selectedArticle.article6_MVC_non_ajuste_trade,
        supportingowners:
          !article6trade && supportingOwners ? supportingOwners : undefined,
  ...
  const certifyproject = async (programmeId: string, certifierId: number) => {
    try {
      const response: any = await put("national/programme/certify", {
      ...
      console.log("Error in programme certification - ", error);
    }
  };
  ...
  const on_type_Article6_trade = (
    articlekey: ArticleKey,
    article_value: boolean
  ) => {
    const newSelection: SelectedArticle = {
      article6_8_trade: false,
      article6_4_trade: false,
      article6_2_trade: false,
      article6_MVC_ajuste_trade: false,
                        ...
                        <Row className="row" gutter={[16, 16]}>
                                      ...
                                      <Row className="in-ndc-row">
                                        ...
                                        >
                                          <Radio.Group
                                            size="middle"
                                            value={
                                              selectedArticle.article6_8_trade
                                            }
                                            onChange={(e) =>
                                              on_type_Article6_trade(
                                                "article6_8_trade",
                                                e.target.value
                                              )
                                            }
                                      ...
                                      <Row className="in-ndc-row">
                                        ...
                                        >
                                          <Radio.Group
                                            size="middle"
                                            value={
                                              selectedArticle.article6_4_trade
                                            }
                                            onChange={(e) =>
                                              on_type_Article6_trade(
                                                "article6_4_trade",
                                      ...
                                      <Row className="in-ndc-row">
                                        ...
                                        >
                                          <Radio.Group
                                            size="middle"
                                            value={
                                              selectedArticle.article6_2_trade
                                            }
                                            onChange={(e) =>
                                              on_type_Article6_trade(
                                                "article6_2_trade",
                                      ...
                                      <Row className="in-ndc-row">
                                        ...
                                        >
                                          <Radio.Group
                                            size="middle"
                                            value={
                                              selectedArticle.article6_MVC_ajuste_trade
                                            }
                                            onChange={(e) =>
                                              on_type_Article6_trade(
                                                "article6_MVC_ajuste_trade",
                          ...
                          <Col xl={12} md={24}>
                            <div className="details-part-two">
                              ...
                              <Form.Item
                                label="Zones et polygones du projet(Télechager le template)"
                                name=""
                                valuePropName="fileList"
                                getValueFromEvent={normFile}
                                required={false}
                              >
                                ...
                                <Upload
                                  ...
                                  className="polygone-upload-section"
                                  name="polygone"
                                  action="/upload.do"
                                  multiple={false}
                                  maxCount={1}
                                ...
                                {invalidPolygons.length > 0 && (
                                  <div>
                                    ...
                                    >
                                      {JSON.stringify(
                                        invalidPolygons.map((p) => p.Intitule),
                                        null,
                                        2
                                      )}
                                    </pre>
                              ...
                              >
                                <Select
                                  size="large"
                                  loading={loadingList}
                                  onChange={onChangecertificator}
                                >
                                  {certificatorList.map((certificator: any) => (
                                    <Select.Option
                                      key={certificator.companyId}
                                      value={certificator.companyId}
                                    >
                                      {certificator.name}
                                    </Select.Option>
                                  ))}
