...
export const ProgrammeManagementComponent = (props: any) => {
  ...
  */
  ...
  // Tableau de correspondance des labels en français pour ProgrammeStageMRV
  const labelsFrMRV = {
  ...
  // Tableau de correspondance des labels en français pour ProgrammeStageR
  const labelsFrR = {
    Pending: "En attente",
    Authorised: "Autorisé",
    Approved: "Approuvé",
  ...
  type ProgrammeStageValues = ProgrammeStageMRV | ProgrammeStageR;

  // Utilisation des valeurs des énumérations pour mapper les labels
  const statusOptions = Object.values(stageObject).map(
    (value: ProgrammeStageValues) => ({
      label: enableAddProgramme ? labelsFrMRV[value] : labelsFrR[value],
      value: value,
    })
  );
