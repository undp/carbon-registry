...
export class NationalAccountingService {
	...
	constructor(
		...
		@InjectRepository(CreditAuditLogViewEntity) private creditAuditLogViewRepo: Repository<CreditAuditLogViewEntity>,
	...
	async getTotalStats() {
		...
		const formattedResult = result.reduce((acc, item) => {
			acc[item.type] = { 'credits': item.sumCredits, 'lastUpdated': item.latestTime ? Math.floor(item.latestTime.getTime()) : 0 };
			return acc;
		}, {});
	...
	async getTransactionRecords(
		...
		if (data && data.length > 0) {
			for (const e of data) {
				if (e.country) {
					e.countryName =
						await this.countryService.getCountryName(e.country);
				}
			}
		}
	...
	async getRetiresByCountry(query: QueryDto, abilityCondition: string) {
		...
		dataQueryBuilder = dataQueryBuilder
			.groupBy('log.country')
			.orderBy(
				query?.sort?.key ? query?.sort?.key === 'totalCredits' ?
					`SUM(log.credits)` : query?.sort?.key === 'latestTime' ?
						'MAX("log"."createdTime")' : `"log"."${query?.sort?.key}"` : `SUM(log.credits)`,
				query?.sort?.order ? query?.sort?.order : "DESC"
			);
