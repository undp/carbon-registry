...
export class AuthService {
	...
	async refreshToken(refreshToken: string) {
		...
		try {
			...
			const decoded = this.jwtService.verify(refreshToken, { secret: this.configService.get<string>("jwt.refreshTokenSecret") });
			userId = decoded.userId;
			companyId = decoded.companyId;
		} catch (err) {
			console.log("Unable to verify the refresh token for userId {}, companyId {} due to {}", userId, companyId, err)
			throw new HttpException(
				this.helperService.formatReqMessagesString(
					"jwt expired",
					[]
