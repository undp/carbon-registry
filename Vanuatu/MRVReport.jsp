<%@page import="com.vanuatu.model.common.MRVOutput"%>
...
<head>
	...
	<title>Project MRV Report</title>
	<!-- Global stylesheets -->
...
String selectedProjectId = (String)request.getAttribute("selectedProject");
%>
<body>
	<!-- Page content -->
...
<div class="content">
	...
	<form action = "getMRVOutput" id = "" class="form-horizontal bordered-row" method="post">
    <div class="card border-teal">
		<div class="card-header text-center bg-info-700">
		<h5 class="card-title text-uppercase">MRV Report <%-- (${ghgReportOptions.getUnit()}) --%></h5>
		</div>
<div class="card-body font-size-lg">
    ...
    <div class="col-lg-6">
  	 <select name="projectId" id="projectId" required data-placeholder="--Select--" class="form-control select" data-fouc>
      <option></option>
    ...
    <div class="tab-container">
                                <ul class="nav nav-tabs justify-content-center nav-fill" role="tablist">
                                    <li class="nav-item">
                                        <a class="nav-link active" data-toggle="tab" href="#project" role="tab">Project Details</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" data-toggle="tab" href="#mitigation" role="tab">Mitigation Summary</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" data-toggle="tab" href="#adaptation" role="tab">Adaptation Summary</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" data-toggle="tab" href="#finance" role="tab">Finance Summary</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link" data-toggle="tab" href="#SDG" role="tab">SDG Summary</a>
                                    </li>
                                  </ul>
                                  <hr>
                                  
<div class="tab-content">
<div class="tab-pane active fade show" id="project" role="tabpanel">
<br>
							...
							<tr class="bg-info-700">
								...
								<th>Value</th>
									...
									<td style="width:10px">1</td>
										<td>Project Title</td>
										<td>${output.getProject().getProjectTitle()}</td>
									</tr>
									<tr>
									<td>2</td>
										<td>Project Cost (USD)</td>
										<td>${output.getProject().getCostAmount()}</td>
									</tr>
									<tr>
									<td>3</td>
										<td>Project Location</td>
										<td>${output.getProject().getLocation()}</td>
									</tr>
									
									<tr>
									<td>4</td>
										<td>Implementing Agency</td>
										<td>${output.getProject().getAgency()}</td>
									</tr>
									<tr>
									<td>5</td>
										<td>Executing Agency</td>
										<td>${output.getProject().getOtherParty()}</td>
									</tr>
									<tr>
									<td>6</td>
										<td>Commissioning Date</td>
										<td>${output.getProject().getCommissioningDate()}</td>
									</tr>
									
									<tr>
									<td>7</td>
										<td>Operation Lifetime (years)</td>
										<td>${output.getProject().getLifetime()}</td>
									</tr>
									</tbody>
									</table></div>
   </div></div></div>
   
   <div class="tab-pane fade show" id="mitigation" role="tabpanel">
   <br>
   ...
   <div class="card border-grey">
    ...
    <h6 class="card-title text-uppercase text-teal-700">Mitigation Performance</h6>
    <c:if test = "${output.getProject().getDivision() ne 'Adaptation' }">
    <div class="table-responsive">
	                            <table id="mitigationTable" class="table table-bordered datatable-button-html5-basic">
	                            	  
									<thead>
									<tr class="bg-info-700">
									  <th>Mitigation Summary</th>
									  <th>Unit</th>
									  <th>Expected Value</th>
									  <c:forEach var="output" varStatus="loop" items="${output.getActualGhgReductionMap()}">
									  <c:set var="year" value="${output.key}"/>
									  <th>${output.key}</th>
									  </c:forEach>
									...
									<tbody>
									<tr class="">
											<td>GHG Emissions Achieved (tCO2e)</td>
											<td>tCO<sub>2</sub>e</td>
											<td>${output.expectedGhgReduction}</td>
										<c:forEach var="entry" varStatus="loop" items="${output.getActualGhgReductionMap()}">
											<td>${entry.value}</td>
										</c:forEach>
										
									</tr>
									<c:forEach var="entry" varStatus="loop" items="${output.getMitigationInputIndicatorMap()}">
									<tr>
												<td>${entry.key}</td>
												<td>${entry.value.unit}</td>
												<td>${entry.value.expectedValue}</td>
												
											<c:forEach var="monitor" varStatus="loop" items="${output.getMitigationMonitoringIndicatorMap()}">
												<c:forEach var="list" varStatus="loop" items="${monitor.value}">
														<c:if test = "${list.indicator eq entry.key }">
															<td>${list.expectedValue}</td>
														</c:if>
												</c:forEach>
											</c:forEach>
									...
									<c:if test = "${output.getProject().getDivision() eq 'Adaptation' }">
									<span>This is an Adaptation Action. No Mitigation data is applicable for this action.</span>
									</c:if>
									</div></div></div>
                 
	<div class="tab-pane fade show" id="adaptation" role="tabpanel">
	<br>
	<div class="card border-grey">
   <div class="card-body">  
    <h6 class="card-title text-uppercase text-teal-700">Adaptation Performance</h6>
    <c:if test = "${output.getProject().getDivision() ne 'Mitigation' }">
    <div class="table-responsive">
    <table id="adaptationTable2" class="table table-bordered datatable-button-html5-basic" >
							<thead>
							<tr class="bg-info-700">
									<th>Quantitative Impacts</th>
									<th>Expected Value</th>
									 <c:forEach var="entry" items="${output.actualAdapMap}" varStatus="loop">
										<th>${entry.key}</th>
									</c:forEach> 
							</tr>
							</thead>
							<tbody>
							<c:forEach var="entry2" items="${output.expectedAdapMap}" varStatus="loop">
								<tr>
									<td class="bg-light"><b>${output.adaptationCategoryMap.get(entry2.key)}</b></td>
								...
								<c:forEach var="questionMap" items="${entry2.value}" varStatus="loop">
								<tr>
										<td>${questionMap.key}</td>
										<td>${questionMap.value}</td>
										
										<c:forEach var="map" items="${output.actualAdapMap}" varStatus="loop">
												
												<c:if test = "${map.value.containsKey(questionMap.key)}">
												 	<td>${map.value.get(questionMap.key)}</td>
											  	</c:if>   
					...
					<div class="table-responsive">
						<table id="adaptationTable1" class="table table-bordered">
							<thead>
							<tr class="bg-info-700">
							<th>Qualitative Impacts</th>
							</tr>
							</thead>
							<tbody>
							<tr>	
								<td>
								 <c:forEach var="setValue" items="${output.adapImpactSummaries}" varStatus="loop">${setValue}
								 		<%-- <img src="images/${setValue}.jpg" alt="" height=40 width=40></img> --%>
									</c:forEach> 
								</td>
							</tr>
							</tbody>
							...
							<c:if test = "${output.getProject().getDivision() eq 'Mitigation' }">
									<span>This is a Mitigation Action. No Adaptation data is applicable for this action.</span>
									</c:if>
							</div></div></div>	
                
					<div class="tab-pane fade show" id="finance" role="tabpanel">
					<br>  
					 <div class="card border-grey">
				   <div class="card-body">  
				      <h6 class="card-title text-uppercase text-teal-700">Climate Finance Summary</h6>
									<div class="table-responsive">
										<table id="financeTable" class="table table-bordered datatable-button-html5-basic">
													<thead>
													<tr class="bg-info-700">
													  <th>Finance Data</th>
													  <c:forEach var="entry" varStatus="loop" items="${output.getFinanceYears()}">
													  	<c:set var="year2" value="${entry}"/>
													  	<th>${entry}</th>
													  </c:forEach>
													...
													<tr>
														<td>Expected Spend (USD)</td>
														<c:forEach var="year" varStatus="loop" items="${output.getFinanceYears()}">
															<td>${output.getDisbursementMap().get(year)}</td>
														</c:forEach>
													...
													<tr>
														...
														<c:forEach var="year" varStatus="loop" items="${output.getFinanceYears()}">
															<td>${output.getFinanceMonitoringMap().get(year)}</td>
														</c:forEach>
										...
										<div class="tab-pane fade show" id="SDG" role="tabpanel">
	<br>
	<div class="card border-grey">
   <div class="card-body">  
    <h6 class="card-title text-uppercase text-teal-700">SDG Performance</h6>
    <div class="table-responsive">
						<table id="sdgTable2" class="table table-bordered datatable-button-html5-basic">
						<thead>
						<tr class="bg-info-700">
								<th>Quantitative Impacts</th>
								<th>Expected Value</th>
								 <c:forEach var="entry" items="${output.actualSdgMap}" varStatus="loop">
									<th>${entry.key}</th>
								</c:forEach> 
						</tr>
						</thead>
						<tbody>
							<c:forEach var="entry2" items="${output.expectedSdgMap}" varStatus="loop">
							<tr>
								<td class="bg-light" colspan="${output.actualSdgMap.size()+2}"><b>${output.sdgCategoryMap.get(entry2.key)}</b></td>
							</tr>
								<c:forEach var="questionMap" items="${entry2.value}" varStatus="loop">
							    <tr>
									<td>${questionMap.key}</td>
								...
								<c:forEach var="map" items="${output.actualSdgMap}" varStatus="loop">
								
								<c:set var = "questionMapMon" scope = "session" value = "${map.value}"/>
								
										<c:choose>
										<c:when test = "${questionMapMon.containsKey(questionMap.key)}">
										
										 	<td>${questionMapMon.get(questionMap.key)}</td>
									  	</c:when>   
										<c:otherwise>
											<td>0.0</td>		
										</c:otherwise>
							...
							<br>
					<div class="table-responsive">
						<table id="sdgTable1" class="table table-bordered">
						<thead>
						<tr class="bg-info-700">
						<th colspan="2" >Qualitative Impacts</th>
						</tr>
						</thead>
						<tbody>
						<tr>	
						<td>
						 <c:forEach var="setValue" items="${output.sdgImpactSummaries}" varStatus="loop">
								<img src="global_assets/images/SDG/${setValue}.jpg" alt="" height=40 width=40></img>
							</c:forEach> 
						</td>
					  ...
					  <button class="btn btn-info" id="btnExportCSV" onclick="tablesToExcel(['data-table'], 'myfile.xls');">
   ...
   var _componentDatatableButtonsHtml5 = function() {
       ...
       $.extend( $.fn.dataTable.defaults, {
           ...
           language: {
               ...
               lengthMenu: '<span><b>Export</b></span> ',
