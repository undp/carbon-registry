...
Map<String, GHGInputOil> existingGHGInputOil = null;

if(request.getAttribute("existingGHGInputOil")!=null){
	existingGHGInputOil = (Map<String, GHGInputOil>)request.getAttribute("existingGHGInputOil");
...
<div class="card border-pink">
	<div class="card-header bg-pink-600">
		<h5 class="card-title text-uppercase text-center">GHG Inventory - Fugitive Emissions from Fuels - Oil</h5>
	</div>

	<div class="card-body">
	<form action="saveGHGInputOil" method="post" onsubmit="return setValues();" class="form-horizontal bordered-row" id="ghgInputForm" data-parsley-validate="">
...
<div class="card-body font-size-lg">
  ...
  <hr>            
                    ...
                    <div class="card-body">
                             ...
                             <h6 class="card-title">Oil - Fugitive Emissions Data</h6>
      <div class="table-responsive">
						<table id="oilTable" class="table table-bordered table-striped">
                           ...
                           <thead>
							...
							<td>All</td>
							<td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">${existingGHGInputOil.get("Oil Production").getAmount()}</td>
							<td>10<sup>3</sup> m<sup>3</sup> of Total Oil Produced</td>
							<td contenteditable='true'>${existingGHGInputOil.get("Oil Production").getReference()}</td>
							</tr>
							
							<tr>
                            <td>Oil Upgraded</td>
							<td>All</td>
							<td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">${existingGHGInputOil.get("Oil Upgraded").getAmount()}</td>
							<td>10<sup>3</sup> m<sup>3</sup> of Oil Upgraded</td>
							<td contenteditable='true'>${existingGHGInputOil.get("Oil Upgraded").getReference()}</td>
							</tr>
							
							<tr>
                            <td>Oil Transported (Pipelines)</td>
							<td>All</td>
							<td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">${existingGHGInputOil.get("Oil Transported (Pipelines)").getAmount()}</td>
							<td>10<sup>3</sup> m<sup>3</sup> of Oil Transported (Pipelines)</td>
							<td contenteditable='true'>${existingGHGInputOil.get("Oil Transported (Pipelines)").getReference()}</td>
							</tr>
							
							<tr>
                            <td>Oil Transported (Tanker Trucks and Rail Cars)</td>
							<td>Venting</td>
							<td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">${existingGHGInputOil.get("Oil Transported (Tanker Trucks and Rail Cars)").getAmount()}</td>
							<td>10<sup>3</sup> m<sup>3</sup> of Oil Transported (Tanker Trucks)</td>
							<td contenteditable='true'>${existingGHGInputOil.get("Oil Transported (Tanker Trucks and Rail Cars)").getReference()}</td>
							</tr>
...
<div class="col-sm-12">
<textarea class="form-control" id="remarks" name="remarks">${existingGHGInputOil.get("Oil Production").getRemarks()}</textarea>
     </div>
</div>
...
function setValues(){
    ...
    for (i = 1; i < rowLength; i++){
       ...
       for(var j = 0; j < cellLength; j++){
           ...
           if(j==0){
        	   name = "ghgInputOilList["+x+"].category";
           }
           if(j==1){
        	   name = "ghgInputOilList["+x+"].subCat";
           }
           if(j==2){
        	   name = "ghgInputOilList["+x+"].amount";
           }
           if(j==4){
        	   name = "ghgInputOilList["+x+"].reference";
           }
...
function getData(){
	...
	var url = '/getGHGInputOilExisting?sector=' + sector + '&subSector='+subSector  + '&category='+category + '&subCategory='+subCategory + '&year='+year + '&status=ANY';
...
function setStatus(id){
	...
	<% if(existingGHGInputOil!=null && !existingGHGInputOil.isEmpty()) { %>
	document.getElementById('dataId').value = "<%=existingGHGInputOil.get("Oil Production").getDataId()%>";
	<% } %>
}
...
<% if(existingGHGInputOil!=null && !existingGHGInputOil.isEmpty()) {
	%>
		...
		document.getElementById('calcApproach').value = "<%=existingGHGInputOil.get("Oil Production").getCalcApproach()%>";
	<%
		} else {
	%>
