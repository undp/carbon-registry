...
List<GHGInputLime> existingGHGInputLime = null;

if(request.getAttribute("existingGHGInputLime")!=null){
	existingGHGInputLime = (List<GHGInputLime>)request.getAttribute("existingGHGInputLime");
...
<div class="card border-pink">
	<div class="card-header bg-pink-600">
		<h5 class="card-title text-uppercase text-center">GHG Inventory - Lime</h5>
	</div>

	<div class="card-body">
	<form action="saveGHGInputLime" method="post" onsubmit="return setValues();" class="form-horizontal bordered-row" id="ghgInputForm" data-parsley-validate="">
...
<div class="form-group">
      				<div class="card">
                    <div class="card-body">
      <h6 class="card-title">Lime Data</h6>
					<div class="table-responsive">
							<table id="limeTable" class="table table-bordered datatable-button-html5-basic">
						<thead>
                            <tr class="bg-pink-600">
                            	<th>Lime Type</th>
                                <th>Amount of Lime Produced (tonnes)</th>
                                <th>Reference</th>
                                <th>Select</th>
                             </tr>
                            </thead>
                            <tbody>
	                           <c:forEach var="output" varStatus="loop" items="${existingGHGInputLime}">
	                            <tr>
	                            		<td contenteditable='true'>${output.getLimeType()}</td>
                        ...
                        <hr>
                          <h6 class="card-title">Add Lime Data</h6>
                        <div class="form-group row">
   
   <div class="col-lg-3 font-size-sm">Lime Type
   <input id="limeType" class="form-control">
  </div>
    <div class="col-lg-3 font-size-sm">Amount Produced (tonnes)
...
<div class="col-sm-12">
<textarea class="form-control" id="remarks" name="remarks">${existingGHGInputLime[0].getRemarks()}</textarea>
     </div>
</div>
...
function setValues(){
    var table = document.getElementById('limeTable');
    ...
    for (i = 1; i < rowLength; i++){
       ...
       for(var j = 0; j < cellLength; j++){
           ...
           if(j==0){
        	   name = "ghgInputLimeList["+x+"].limeType";
           }
           if(j==1){
        	   name = "ghgInputLimeList["+x+"].amount";
        	}
           if(j==2){
        	   name = "ghgInputLimeList["+x+"].reference";
       	   }
...
function getData(){
	...
	var url = '/getGHGInputLimeExisting?sector=' + sector + '&subSector='+subSector  + '&category='+category + '&subCategory='+subCategory + '&year='+year + '&status=ANY';
...
function setStatus(id){
	...
	<% if(existingGHGInputLime!=null && !existingGHGInputLime.isEmpty()) { %>
	document.getElementById('dataId').value = "<%=existingGHGInputLime.get(0).getDataId()%>";
	<% } %>
}
	...
	%>
		...
		document.getElementById('calcApproach').value = "<%=existingGHGInputLime.get(0).getCalcApproach()%>";
	<%
		} else {
	%>
...
$(document).ready(function(){
    ...
    $(".add-row1").click(function(){
    	var limeType = $("#limeType").val();
    	var amount= $("#amount").val();
        var reference = $("#ref").val();
        var markup = "<tr><td>" + limeType + "</td><td contenteditable='true'>" + amount+ "</td><td contenteditable='true'>" + reference + "</td><td><input type='checkbox' name='record'></td></tr>";
        $("#limeTable tbody").append(markup);
    });
    
    // Find and remove selected table rows
    $(".delete-row1").click(function(){
        $("#limeTable tbody").find('input[name="record"]').each(function(){
            if($(this).is(":checked")){
