<%@page import="com.sudan.model.ghg.afolu.land.GHGInputWetland"%>
...
List<GHGInputWetland> existingGHGInputWetland = null;

if(request.getAttribute("existingGHGInputWetland")!=null){
	existingGHGInputWetland = (List<GHGInputWetland>)request.getAttribute("existingGHGInputWetland");
...
<div class="card border-pink">
	<div class="card-header bg-pink-600">
		<h5 class="card-title text-uppercase text-center">GHG Inventory - Wetlands</h5>
	</div>

	<div class="card-body">
	<form action="saveGHGInputWetland" method="post" onsubmit="return setValues();" class="form-horizontal bordered-row" id="ghgInputForm" data-parsley-validate="">
...
<div class="form-group">
      				<div class="card">
                    <div class="card-body">
      <h6 class="card-title">Wetlands Data</h6>
					<div class="table-responsive">
						<table id="ghgTable1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                            <c:choose>
							<c:when test = "${empty existingGHGInputWetland}">
                            <tr>
                                <td>Wetlands Remaining Wetlands </td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
								<td contenteditable='true'></td>
                                </tr>
                                <tr>
                                <td>Forest Land converted to Wetlands</td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
								<td contenteditable='true'></td>
                                </tr>
                                <tr>
                                <td>Cropland converted to Wetlands</td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
								<td contenteditable='true'></td>
                                </tr>
                                <tr>
                                <td>Grassland converted to Wetlands</td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
								<td contenteditable='true'></td>
                                </tr>
                                <tr>
                                <td>Settlements converted to Wetlands</td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
								<td contenteditable='true'></td>
                                </tr>
                                <tr>
                                <td>Other Land converted to Wetlands</td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
                                <td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true""></td>
								<td contenteditable='true'></td>
                                </tr>
							...
							<c:forEach var="output" varStatus="loop" items="${existingGHGInputWetland}">
							    <tr>
...
<div class="col-sm-12">
<textarea class="form-control" id="remarks" name="remarks">${existingGHGInputWetland[0].getRemarks()}</textarea>
     </div>
</div>
...
function setValues(){
    ...
    for (i = 1; i < rowLength; i++){
       ...
       for(var j = 0; j < cellLength; j++){
           ...
           var name ;
           if(j==0 ){
        	   name = "gHGInputWetlandList["+x+"].landcategory";
           }
           if(j==1){
        	   name = "gHGInputWetlandList["+x+"].area";
           }
           if(j==2){
        	   name = "gHGInputWetlandList["+x+"].emissions";
           }
           if(j==3){
         	  name = "gHGInputWetlandList["+x+"].reference";
            }
...
function getData(){
	...
	var url = '/getGHGInputWetlandExisting?sector=' + sector + '&subSector='+subSector  + '&category='+category + '&subCategory='+subCategory + '&year='+year + '&status=ANY';
...
function setStatus(id){
	...
	<% if(existingGHGInputWetland!=null && !existingGHGInputWetland.isEmpty()) { %>
	document.getElementById('dataId').value = "<%=existingGHGInputWetland.get(0).getDataId()%>";
	<% } %>
}
...
<% if(existingGHGInputWetland!=null && !existingGHGInputWetland.isEmpty()) {
	%>
		...
		document.getElementById('calcApproach').value = "<%=existingGHGInputWetland.get(0).getCalcApproach()%>";
	<%
		} else {
	%>
