...
Map<String, GHGInputGas> existingGHGInputGas = null;

if(request.getAttribute("existingGHGInputGas")!=null){
	existingGHGInputGas = (Map<String, GHGInputGas>)request.getAttribute("existingGHGInputGas");
...
<div class="card border-pink">
	<div class="card-header bg-pink-600">
		<h5 class="card-title text-uppercase text-center">GHG Inventory - Fugitive Emissions from Fuels - Natural Gas</h5>
	</div>

	<div class="card-body">
	<form action="saveGHGInputGas" method="post" onsubmit="return setValues();" class="form-horizontal bordered-row" id="ghgInputForm" data-parsley-validate="">
...
<div class="card-body font-size-lg">
  ...
  <hr>            
                    ...
                    <div class="card-body">
                             ...
                             <h6 class="card-title">Natural Gas - Fugitive Emissions Data</h6>
      <div class="table-responsive">
						<table id="gasTable" class="table table-bordered table-striped">
                           ...
                           <thead>
							...
							<td>All</td>
							<td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">${existingGHGInputGas.get("Gas Production").getAmount()}</td>
							<td>million m<sup>3</sup> of Gas Produced</td>
							<td contenteditable='true'>${existingGHGInputGas.get("Gas Production").getReference()}</td>
							</tr>
							
							<tr>
                            <td>Gas Transmission and Storage</td>
							<td>All</td>
							<td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">${existingGHGInputGas.get("Gas Transmission and Storage").getAmount()}</td>
							<td>million m<sup>3</sup> of Marketable Gas Produced</td>
							<td contenteditable='true'>${existingGHGInputGas.get("Gas Transmission and Storage").getReference()}</td>
							</tr>
							
							<tr>
                            <td>Gas Distribution</td>
							<td>All</td>
							<td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">${existingGHGInputGas.get("Gas Distribution").getAmount()}</td>
							<td>million m<sup>3</sup> of Gas Sales</td>
							<td contenteditable='true'>${existingGHGInputGas.get("Gas Distribution").getReference()}</td>
							</tr>
							
							<tr>
                            <td>Natural Gas Liquid Transport (LPG)</td>
							<td>LPG</td>
							<td contenteditable='true' class ="input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">${existingGHGInputGas.get("Natural Gas Liquid Transport (LPG)").getAmount()}</td>
							<td>million m<sup>3</sup> of LPG Produced</td>
							<td contenteditable='true'>${existingGHGInputGas.get("Natural Gas Liquid Transport (LPG)").getReference()}</td>
							</tr>
							
                            </tbody>
                           </table>
                           </div></div>
                           <a class ="text-danger">* Emissions from gas processing are estimated using default emission factors and gas production
                           <br>
                           * Emissions from LNG transport are estimated using marketable gas produced</a>
                           </div></div>   
	     
	     <div class="form-group">
                <span class="col-sm-3 control-label">Remarks</span>
<div class="col-sm-12">
<textarea class="form-control" id="remarks" name="remarks">${existingGHGInputGas.get("Gas Production").getRemarks()}</textarea>
     </div>
</div>
...
function setValues(){
    ...
    for (i = 1; i < rowLength; i++){
       ...
       for(var j = 0; j < cellLength; j++){
           ...
           if(j==0){
        	   name = "ghgInputGasList["+x+"].category";
           }
           if(j==1){
        	   name = "ghgInputGasList["+x+"].subCat";
           }
           if(j==2){
        	   name = "ghgInputGasList["+x+"].amount";
           }
           if(j==4){
        	   name = "ghgInputGasList["+x+"].reference";
           }
...
function getData(){
	...
	var url = '/getGHGInputGasExisting?sector=' + sector + '&subSector='+subSector  + '&category='+category + '&subCategory='+subCategory + '&year='+year + '&status=ANY';
...
function setStatus(id){
	...
	<% if(existingGHGInputGas!=null && !existingGHGInputGas.isEmpty()) { %>
	document.getElementById('dataId').value = "<%=existingGHGInputGas.get("Gas Production").getDataId()%>";
	<% } %>
}
...
<% if(existingGHGInputGas!=null && !existingGHGInputGas.isEmpty()) {
	%>
		...
		document.getElementById('calcApproach').value = "<%=existingGHGInputGas.get("Gas Production").getCalcApproach()%>";
	<%
		} else {
	%>
