...
List<GHGInputLubricant> existingGHGInputLubricant = null;

if(request.getAttribute("existingGHGInputLubricant")!=null){
	existingGHGInputLubricant = (List<GHGInputLubricant>)request.getAttribute("existingGHGInputLubricant");
...
<div class="card border-pink">
	<div class="card-header bg-pink-600">
		<h5 class="card-title text-uppercase text-center">GHG Inventory - Lubricant Use</h5>
	</div>

	<div class="card-body">
	<form action="saveGHGInputLubricant" method="post" onsubmit="return setValues();" class="form-horizontal bordered-row" id="ghgInputForm" data-parsley-validate="">
...
<div class="form-group">
      ...
      <h6 class="card-title">Lubricant Use Data</h6>
					<div class="table-responsive">
							<table id="lubricantTable" class="table table-bordered datatable-button-html5-basic">
						<thead>
                            ...
                            <tbody>
	                           <c:forEach var="output" varStatus="loop" items="${existingGHGInputLubricant}">
	                            <tr>
	                            		<td contenteditable='true'>${output.getLubricant()}</td>
                        ...
                        <hr>
                          <h6 class="card-title">Add Lubricant</h6>
                        <div class="form-group row">
   
   <div class="col-lg-2 font-size-sm">Lubricant Name
   <input id="lubricant" class="form-control">
  </div>
    <div class="col-lg-2 font-size-sm">Amount Consumed 
   <input id = 'amount'  class="form-control input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">
    <span class="form-text text-muted text-center">(TJ)</span>
  </div>
  <div class="col-lg-2 font-size-sm">Carbon Content 
   <input id = 'carbon'  class="form-control input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">
   <span class="form-text text-muted text-center">(tonne C/TJ)</span>
  </div>
  <div class="col-lg-2 font-size-sm">Fraction Oxidised
   <input id = 'fraction'  class="form-control input-mask-trigger" data-inputmask="'alias': 'decimal', 'autoGroup': true" im-insert="true">
   <span class="form-text text-muted text-center">(fraction)</span>
  </div>
...
<div class="col-sm-12">
<textarea class="form-control" id="remarks" name="remarks">${existingGHGInputLubricant[0].getRemarks()}</textarea>
     </div>
</div>
...
function setValues(){
    var table = document.getElementById('lubricantTable');
    ...
    for (i = 1; i < rowLength; i++){
       ...
       for(var j = 0; j < cellLength; j++){
           ...
           if(j==0){
        	   name = "ghgInputLubricantList["+x+"].lubricant";
           }
           if(j==1){
        	   name = "ghgInputLubricantList["+x+"].amount";
        	}
           if(j==2){
        	   name = "ghgInputLubricantList["+x+"].carbon";
       	   }
           if(j==3){
        	   name = "ghgInputLubricantList["+x+"].fraction";
       	   }
           if(j==4){
        	   name = "ghgInputLubricantList["+x+"].reference";
       	   }
...
function getData(){
	...
	var url = '/getGHGInputLubricantExisting?sector=' + sector + '&subSector='+subSector  + '&category='+category + '&subCategory='+subCategory + '&year='+year + '&status=ANY';
...
$(document).ready(function(){
    ...
    $(".add-row1").click(function(){
    	var lubricant = $("#lubricant").val();
    	var amount= $("#amount").val();
        var carbon = $("#carbon").val();
        var fraction = $("#fraction").val();
        var reference = $("#ref").val();
        var markup = "<tr><td>" + lubricant + "</td><td contenteditable='true'>" + amount+ "</td><td contenteditable='true'>" + carbon + "</td><td contenteditable='true'>" + fraction + "</td><td contenteditable='true'>" + reference + "</td><td><input type='checkbox' name='record'></td></tr>";
        $("#lubricantTable tbody").append(markup);
    });
    
    // Find and remove selected table rows
    $(".delete-row1").click(function(){
        $("#lubricantTable tbody").find('input[name="record"]').each(function(){
            if($(this).is(":checked")){
...
function setStatus(id){
	...
	<% if(existingGHGInputLubricant!=null && !existingGHGInputLubricant.isEmpty()) { %>
	document.getElementById('dataId').value = "<%=existingGHGInputLubricant.get(0).getDataId()%>";
	<% } %>
}
...
<% if(existingGHGInputLubricant!=null && !existingGHGInputLubricant.isEmpty()) {
	%>
		...
		document.getElementById('calcApproach').value = "<%=existingGHGInputLubricant.get(0).getCalcApproach()%>";
	<%
		} else {
	%>
