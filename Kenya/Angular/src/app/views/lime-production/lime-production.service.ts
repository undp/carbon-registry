...
import { MRV_INVENTORY_YEAR, MRV_FUEL_TYPE, MRV_FUEL_BY_FUEL_TYPE, MRV_APPROVALS, MRV_GHG_IPPU_LIME } from '../../app.constants';
import { ApiResponseModel } from '../model/api.response.model';

@Injectable({
  providedIn: 'root'
})
export class LimeProductionService {
  ...
  public getEData(obj){
    ...
    return this.http.get<ApiResponseModel>(MRV_GHG_IPPU_LIME , {headers : this.getHeader(), params : params});
  }

  public saveElectricityGeneration(body){
    return this.http.post<ApiResponseModel>(`${MRV_GHG_IPPU_LIME}` , body, {headers : this.getHeader()});
  }
