...
/** @class */
function () {
  function OthersComponent(_fb, _os, _activatedRoute, _snackBar, _router, appService) {
    this._fb = _fb;
    this._os = _os;
  ...
  OthersComponent.prototype.updateDataStaus = function (status) {
    ...
    this._os.updateDataStatus(obj).subscribe(function (res) {
      if (res.statusCode == 200) {
        ...
        _this._router.navigate(['ghg/energy/Sectoral_Approach/Others']);
      } else {
        _this.openSnackBar(res.message, 'error');
      }
    }, function (err) {
  ...
  OthersComponent = OthersComponent_1 = __decorate([core_1.Component({
    selector: 'app-others',
