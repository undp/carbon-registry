...
export class ManufacturingComponent implements OnInit {
  ...
  constructor(private readonly _fb: FormBuilder,
    private readonly _ms: ManufacturingService,
  ...
  getYears() {
    this._ms.getInventoryYears().subscribe((res: any) => {
  ...
  saveElectricityGeneration() {
    ...
    this.appService.saveRecord(obj).subscribe((res: any) => {
      if (res.statusCode == 200) {
        ...
        this.formGroup.controls['subCategory'].reset();
        this.dataList = [];
        this.dataSource.data = this.dataList;
        this.newlyAddedList = [];
        this.newDataSource.data = this.newlyAddedList;
  ...
  updateDataStaus(status) {
    ...
    this._ms.updateDataStatus(obj).subscribe(res => {
      if (res.statusCode == 200) {
        this.utilityService.openSnackBar(res.message, MrvConstants.LOG_SUCCESS);
