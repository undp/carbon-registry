...
import { LimeProductionComponent } from '../lime-production/lime-production.component';
import { LimeProductionService } from '../lime-production/lime-production.service';
...
export class LubricantUseComponent implements OnInit{
  formGroup: FormGroup;
  yearList: any;
  // sector details
  sectorArr:string[] = [];
  ...
  lubricantNameCtrl = new FormControl();
  ...
  columnNames = [
    { id: "lubricantName", value: "Lubricant Type", formControl: this.lubricantNameCtrl },
  ...
  filteredValues = {
    lubricantName: '', amountConsumed: '', reference: '', carbonContent: '', fractionOxidised :''
  };
  ...
  constructor(private readonly _fb: FormBuilder,
    private readonly _lus: LubricantUseService,
  ...
  setFormValue(data: any) {
    ...
    this.dataList = data.lubricantData;
    this.dataSource.data = this.dataList;
  }
  ngOnInit() {
    this.yearList = [];
    ...
    this.lubricantNameCtrl.valueChanges.subscribe((positionFilterValue) => {
      this.filteredValues['lubricantName'] = positionFilterValue;
      this.dataSource.filter = JSON.stringify(this.filteredValues);
      this.filteredValues['topFilter'] = false;
      this.dataSource.filterPredicate = this.customFilterPredicate();
    });
  ...
  getYears() {
    this._lus.getInventoryYears().subscribe((res: any) => {
  ...
  public customFilterPredicate() {
    const myFilterPredicate = function (data: any, filter: string): boolean {
      ...
      let lubricantName = data.lubricantName.toString().trim().toLowerCase().indexOf(searchString.lubricantName.toLowerCase()) !== -1
  ...
  addNewRow() {
    ...
    this.newlyAddedList.push({
      lubricantName: '', amountConsumed: 0,reference: '', fractionOxidised :0.2, carbonContent: 20
    });
    this.newDataSource.data = this.newlyAddedList
  }
  ...
  updateDataStaus(status){
    ...
    this._lus.updateDataStatus(obj).subscribe(res=>{
      if(res.statusCode ==200){
        this.utilityService.openSnackBar(res.message, MrvConstants.LOG_SUCCESS);
