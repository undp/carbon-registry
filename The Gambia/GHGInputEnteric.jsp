...
List<GHGInputEnteric> existingGHGInputEnteric = null;

if(request.getAttribute("existingGHGInputEnteric")!=null){
	existingGHGInputEnteric = (List<GHGInputEnteric>)request.getAttribute("existingGHGInputEnteric");
...
<div class="card border-slate">
	<div class="card-header bg-slate-700">
		<h5 class="card-title text-uppercase text-center">GHG Inventory - Enteric Fermentation</h5>
	</div>

	<div class="card-body">
	<form action="saveGHGInputEnteric" method="post" onsubmit="return setValues();" class="form-horizontal bordered-row" id="ghgInputForm" data-parsley-validate="">
...
<div class="form-group">
      				<div class="card">
                    <div class="card-body">
      <h6 class="card-title">Enteric Fermentation</h6>
      <div class="table-responsive">
						<table id="livestockTable" class="table table-bordered table-striped">
                            ...
                            <tbody>
                               <c:forEach var="output" varStatus="loop" items="${existingGHGInputEnteric}">
                               <tr>
                               <td>${output.getAnimalcategory()}</td>
                               <td>${output.getAnimals()}</td>
                               <td>${output.getEntericEmissions()}</td>
                               </tr>
                               </c:forEach>
                            </tbody>
                           </table> 
...
<div class="col-sm-12">
<textarea class="form-control" id="remarks" name="remarks" placeholder="${existingGHGInputEnteric[0].getRemarks()}"></textarea>
     </div>
</div>
...
function setValues(){
    ...
    for (i = 1; i < rowLength; i++){
       ...
       for(var j = 0; j < cellLength; j++){
           ...
           if(j==0){
        	   name = "ghgInputEntericList["+x+"].animalcategory";
           }
           if(j==1){
        	   name = "ghgInputEntericList["+x+"].animals";
        	   
           }
           if(j==2){
        	   name = "ghgInputEntericList["+x+"].entericEmissions";
       	   }
...
function getData(){
	...
	var url = '/getGHGInputEntericExisting?sector=' + sector + '&subSector='+subSector  + '&category='+category + '&subCategory='+subCategory + '&year='+year + '&status=ANY';
...
function setStatus(id){
	...
	<% if(existingGHGInputEnteric!=null && !existingGHGInputEnteric.isEmpty()) { %>
	document.getElementById('dataId').value = "<%=existingGHGInputEnteric.get(0).getDataId()%>";
	<% } %>
}
