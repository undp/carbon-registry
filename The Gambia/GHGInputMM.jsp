...
List<GHGInputMM> existingGHGInputMM = null;

if(request.getAttribute("existingGHGInputMM")!=null){
	existingGHGInputMM = (List<GHGInputMM>)request.getAttribute("existingGHGInputMM");
...
<div class="card border-slate">
	<div class="card-header bg-slate-700">
		<h5 class="card-title text-uppercase text-center">GHG Inventory - Manure Management</h5>
	</div>

	<div class="card-body">
	<form action="saveGHGInputMM" method="post" onsubmit="return setValues();" class="form-horizontal bordered-row" id="ghgInputForm" data-parsley-validate="">
...
<div class="form-group">
      				<div class="card">
                    <div class="card-body">
      <h6 class="card-title">Manure Management</h6>
      <div class="table-responsive">
						<table id="livestockTable" class="table table-bordered table-striped">
                            ...
                            <tbody>
                            <tbody>
                               <c:forEach var="output" varStatus="loop" items="${existingGHGInputMM}">
                               <tr>
                               <td>${output.getAnimalcategory()}</td>
                               <td>${output.getAnimals()}</td>
                               <td>${output.getMmEmissions()}</td>
                               </tr>
                               </c:forEach>
                            </tbody>
                            </tbody>
...
<div class="col-sm-12">
<textarea class="form-control" id="remarks" name="remarks" placeholder="${existingGHGInputMM[0].getRemarks()}"></textarea>
     </div>
</div>
...
function setValues(){
    ...
    for (i = 1; i < rowLength; i++){
       ...
       for(var j = 0; j < cellLength; j++){
           ...
           if(j==0){
        	   name = "ghgInputMMList["+x+"].animalcategory";
           }
           if(j==1){
        	   name = "ghgInputMMList["+x+"].animals";
        	   
           }
           if(j==2){
        	   name = "ghgInputMMList["+x+"].mmEmissions";
       	   }
...
function getData(){
	...
	var url = '/getGHGInputMMExisting?sector=' + sector + '&subSector='+subSector  + '&category='+category + '&subCategory='+subCategory + '&year='+year + '&status=ANY';
...
function setStatus(id){
	...
	<% if(existingGHGInputMM!=null && !existingGHGInputMM.isEmpty()) { %>
	document.getElementById('dataId').value = "<%=existingGHGInputMM.get(0).getDataId()%>";
	<% } %>
}
<% if(existingGHGInputMM!=null && !existingGHGInputMM.isEmpty()) {
	%>
