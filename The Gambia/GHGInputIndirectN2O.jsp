...
List<GHGInputIndirectN2O> existingGHGInputIndirectN2O = null;

if(request.getAttribute("existingGHGInputIndirectN2O")!=null){
	existingGHGInputIndirectN2O = (List<GHGInputIndirectN2O>)request.getAttribute("existingGHGInputIndirectN2O");
...
<div class="card border-slate">
	<div class="card-header bg-slate-700">
		<h5 class="card-title text-uppercase text-center">GHG Inventory - Indirect N<sub>2</sub>O Emissions from Manure Management</h5>
	</div>

	<div class="card-body">
	<form action="saveGHGInputIndirectN2O" method="post" onsubmit="return setValues();" class="form-horizontal bordered-row" id="ghgInputForm" data-parsley-validate="">
...
<div class="form-group">
      				<div class="card">
                    <div class="card-body">
      <h6 class="card-title">Manure Management - Indirect N<sub>2</sub>O Emissions</h6>
      <div class="table-responsive">
						<table id="indirectTable" class="table table-bordered table-striped">
	                    
                            <thead>
                            <tr class="bg-slate-700">
                                <th>Category</th>
                                <th>Heads</th>
                                <th>GHG Emissions (tCO<sub>2</sub>e)</th>
                            </tr>
                            </thead>
                            <tbody>
                            <c:forEach var="output" varStatus="loop" items="${existingGHGInputIndirectN2O}">
                               <tr>
                               <td>${output.getAnimalcategory()}</td>
                               <td>${output.getAnimals()}</td>
                               <td>${output.getIndirectEmissions()}</td>
                               </tr>
                               </c:forEach>
                            </tbody>
                           </table> 
...
<div class="col-sm-12">
<textarea class="form-control" id="remarks" name="remarks" placeholder="${existingGHGInputIndirectN2O[0].getRemarks()}"></textarea>
     </div>
</div>
...
</div></div></div></div></form></div></div></div></div></div></body>
<script>

function setValues(){
    var table = document.getElementById('indirectTable');
    ...
    for (i = 1; i < rowLength; i++){
       ...
       for(var j = 0; j < cellLength; j++){
           ...
           if(j==0){
        	   name = "ghgInputIndirectList["+x+"].animalcategory";
           }
           if(j==1){
        	   name = "ghgInputIndirectList["+x+"].animals";
        	   
           }
           if(j==2){
        	   name = "ghgInputIndirectList["+x+"].indirectEmissions";
       	   }
...
function getData(){
	...
	var url = '/getGHGInputIndirectN2OExisting?sector=' + sector + '&subSector='+subSector  + '&category='+category + '&subCategory='+subCategory + '&year='+year + '&status=ANY';
...
function setStatus(id){
	...
	<% if(existingGHGInputIndirectN2O!=null && !existingGHGInputIndirectN2O.isEmpty()) { %>
	document.getElementById('dataId').value = "<%=existingGHGInputIndirectN2O.get(0).getDataId()%>";
	<% } %>
}
