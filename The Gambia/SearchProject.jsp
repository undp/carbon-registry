...
<div class="content">
	...
	<div class="card border-slate">
		...
		<h5 class="card-title text-uppercase text-center">Search NDC Action</h5></div>
					...
					<thead>
							...
							<c:forEach var="output" varStatus="loop" items="${projectList}">
								...
								<c:param name="dataId" value="${output.getDataId()}" />
										...
										<td><a href="${urlDel}" onclick="return confirm('Are you sure you want to remove this NDC Action?')" class="icon-bin mr-3 icon-1x"></a></td>
									</tr>
							    </c:forEach>
							</tbody>
						</table>
						<button type="button" onclick = "create();" class="btn btn-link text-info-700 font-size-lg"><i class="icon-plus3 mr-2"></i>Create NDC Action</button>
