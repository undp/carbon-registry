...
const class_test = new role()
var create_test = async(req,res)=>{

    if(isNotEmpty(req.body.Intitule_role,

    ) ){
        // res.status(403).json({ message: 'Empty Token' })
        let data = await class_test.read({Intitule_role:req.body.Intitule_role})
        if(data.length==0){
            let date = new Date()
        let val = await class_test.create({
            
            Intitule_role :req.body.Intitule_role,
            Default:false,
            Date_creation : date,
