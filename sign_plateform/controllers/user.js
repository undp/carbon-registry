...
var create_test = async (req, res) => {
  if (
    isNotEmpty(req.body.Nom, req.body.Prenom, req.body.Email, req.body.Role)
  ) {
    ...
    if (data_email.length != 0) {
      ...
      if (dt_role) {
        ...
        const class_email = new email(
          "Registre carbone Côte d'Ivoire <roger.doffou@skyvisionafrica.com>",
          req.body.Email
        );
        let new_link =
          "http://localhost:3030/myregister/active/" + val._id + "/" + code;
        let dat = await class_email._main("ask_account", new_link, psss);
        console.log(dat);
        isObjectsuccess(val._id)
          ? res.status(200).json({ status: 200, message: "Success", data: val })
...
var login = async (req, res) => {
  if (isNotEmpty(req.body.Email, req.body.Password)) {
    let data_email = await class_test.read({
      Email: req.body.Email,
    ...
    if (data_email.length == 1 && data_email[0].Statut == "actif") {
      res
        .status(200)
        .json({ status: 200, message: "Success", data: data_email[0] });
    } else {
      res.status(401).json({ status: 401, message: "Empty data" });
    }
  } else {
...
var active = async (req, res) => {
  if (isNotEmpty(req.body.Id)) {
    let data_acc = await class_test.read({
      _id: req.body.Id,
...
var stat = async (req, res) => {
  if (isNotEmpty(req.body.Id_user)) {
    ...
    if (data_acc.length == 1) {
      ...
      let documents = [];
      await Promise.all(
        data_acc[0].TypeFile.map(async (elt) => {
      ...
      let mine = {
        signe: documents.filter(
          (doc) =>
            doc.Status === "accepted" && doc.Id_user_sign == req.body.Id_user
        ).length,
        refuse: documents.filter(
          (doc) =>
            doc.Status === "denied" && doc.Id_user_sign == req.body.Id_user
        ).length,
        doc: docus,
      };
