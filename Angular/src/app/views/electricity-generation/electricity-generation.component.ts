...
export class ElectricityGenerationComponent implements OnInit {
  ...
  constructor(private readonly _fb: FormBuilder,
    private readonly _egs: ElectricityGenerationService,
  ...
  getYears() {
    this._egs.getInventoryYears('mannual').subscribe((res: any) => {
  ...
  updateDataStaus(status) {
    ...
    this._egs.updateDataStatus(obj).subscribe(res => {
      if (res.statusCode == 200) {
        this.utilityService.openSnackBar(res.message, 'success');
