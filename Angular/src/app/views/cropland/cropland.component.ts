...
import { ForestLandComponent } from '../forest-land/forest-land.component';
...
export class CroplandComponent implements OnInit {
  // sector details
  sectorArr: string[] = [];
  categoryArr: string[] = [];
  subSectorArr: string[] = [];
  ...
  setFormValue(dataInput: any) {
    ...
    this.dataList.forEach(data => {
      dataInput.croplandData.forEach(element => {
  ...
  private buildForm() {
    this.formGroup = this._fb.group({
      ...
      sector: ['3-Agriculture, Forestry, and Other Land Use', [Validators.required]],
      subSector: ['3.B.2-Cropland', [Validators.required]],
      category: ['3.B-Land', [Validators.required]],
