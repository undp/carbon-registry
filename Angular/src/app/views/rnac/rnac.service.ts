...
import { MRV_INVENTORY_YEAR, MRV_FUEL_TYPE, MRV_FUEL_BY_FUEL_TYPE, MRV_APPROVALS, MRV_GHG_IPPU_LUBRICANT, MRV_GHG_IPPU_RNAC, MRV_SHARED_APP_DATA } from '../../app.constants';
import { ApiResponseModel } from '../model/api.response.model';

@Injectable({
  providedIn: 'root'
})
export class RNACService {
  ...
  public getEData(obj){
    ...
    return this.http.get<ApiResponseModel>(MRV_GHG_IPPU_RNAC , {headers : this.getHeader(), params : params});
  }

  public saveElectricityGeneration(body){
    return this.http.post<ApiResponseModel>(`${MRV_GHG_IPPU_RNAC}` , body, {headers : this.getHeader()});
  }
  ...
  public getGasConsumed() {
    let myParams: HttpParams = new HttpParams().set('key', 'gagConsumed').set('outputValue', 'value');
    return this.http.get<ApiResponseModel>(`${MRV_SHARED_APP_DATA}` , {headers : this.getHeader(), params: myParams});
  }
}