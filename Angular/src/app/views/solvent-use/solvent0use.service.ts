...
import { MRV_INVENTORY_YEAR, MRV_FUEL_TYPE, MRV_FUEL_BY_FUEL_TYPE, MRV_APPROVALS, MRV_GHG_IPPU_LUBRICANT, MRV_GHG_IPPU_SOLVENT, MRV_SHARED_APP_DATA } from '../../app.constants';
import { ApiResponseModel } from '../model/api.response.model';

@Injectable({
  providedIn: 'root'
})
export class SolventUseService {
  ...
  public getEData(obj){
    ...
    return this.http.get<ApiResponseModel>(MRV_GHG_IPPU_SOLVENT , {headers : this.getHeader(), params : params});
  }

  public saveElectricityGeneration(body){
    return this.http.post<ApiResponseModel>(`${MRV_GHG_IPPU_SOLVENT}` , body, {headers : this.getHeader()});
  }
